 ...
 * compressed file from the archive (disk), and writes it into the provided
...
/*
 * Helper for pyi_archive_extract/pyi_archive_extract2fs that extracts a
 * compressed file from the archive (buffer), and writes it into the provided
 * file handle or data buffer. Exactly one of out_fp or out_ptr needs
 * to be valid.
 */
static int
_pyi_archive_extract_compressed_modified(unsigned char *toc_entry_address, const struct TOC_ENTRY *toc_entry, FILE *out_fp, unsigned char *out_ptr)
{
    const size_t CHUNK_SIZE = 8192;
    unsigned char *buffer_in = NULL;
    unsigned char *buffer_out = NULL;
 ...
 * the archive (disk) into the provided (pre-allocated) buffer.
 */
static int
...
/*
 * Helper for pyi_archive_extract that extracts an uncompressed file from
 * the archive (buffer) into the provided (pre-allocated) buffer.
 */
static int
_pyi_archive_extract_uncompressed_modified(unsigned char *toc_entry_address, const struct TOC_ENTRY *toc_entry, unsigned char *out_buf)
{
    const size_t CHUNK_SIZE = 8192;
    unsigned char *buffer;
    uint64_t remaining_size;
...
/*
 * Extract an archive entry (from disk) into data buffer.
...
/*
 * Extract an archive entry (from buffer) into data buffer.
 * Returns pointer to the data (must be freed).
 */
unsigned char *
pyi_archive_extract_modified(const struct ARCHIVE *archive, const struct TOC_ENTRY *toc_entry)
{
    //FILE *archive_fp = NULL;
    ...
    /* Extract */
    if (toc_entry->compression_flag == 1) {
        rc = _pyi_archive_extract_compressed_modified(toc_entry_address, toc_entry, NULL, data);
    } else {
        rc = _pyi_archive_extract_uncompressed_modified(toc_entry_address, toc_entry, data);
    }
    if (rc != 0) {
        free(data);
        data = NULL;
...
/*
 * Perform full back-to-front scan of the buffer to search for the
 * MAGIC pattern of the embedded archive's COOKIE header.
 *
...
pyi_archive_open_modified(const struct EXE_BUFFER *exe_buffer)
{
    ...
    /* Search for the embedded archive's cookie */
    cookie_pos = _pyi_archive_find_pkg_cookie_offset_modified(exe_buffer);
    if (cookie_pos == 0) {
        PYI_DEBUG("LOADER: cannot find cookie!\n");
        goto cleanup;
    }
